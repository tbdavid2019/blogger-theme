...
- **清單排版**：修正 `.post-body li` 換行/符號對齊問題，改用 flex 排列並修正標點間距。
- **主選單重整**：合併「服務 + HuggingFace」為「333服務」，重新整理多個連結與分類。
- **Bot 區塊**：新增「奇門遁甲解盤」「梅花易數解盤」連結。
- **暗黑模式表格**：補上表格暗色背景/字色與表頭強制樣式（含第一列視為表頭的覆蓋）。
- **暗黑模式圖示**：修正切換按鈕的字重，避免顯示錯誤圖示。
